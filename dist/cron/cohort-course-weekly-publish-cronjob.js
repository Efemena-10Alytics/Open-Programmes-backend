...
const updateCohortCourseWeekPublishStatus = async (cohortId) => {
    try {
        ...
        if (lastUnpublishedWeek) {
            ...
            console.log(`Course week updated for cohort course week ID: ${lastUnpublishedWeek.id}`);
        }
        else {
            console.log(`No unpublished cohort course weeks found for cohort ID: ${cohortId}`);
        }
    }
    catch (error) {
        console.error("Detailed error for publishing course cohort weekly with cron:", error);
        return;
    }
};
...
//# sourceMappingURL=cohort-course-weekly-publish-cronjob.js.map