...
const generateVerificationToken = async (email) => {
    ...
    const existingToken = await prismadb_1.prismadb.verificationToken.findFirst({
        where: {
            email,
        },
    });
    ...
    const verificationToken = prismadb_1.prismadb.verificationToken.create({
        data: {
            token,
            email,
            expires,
...
const generatePasswordResetToken = async (email) => {
    ...
    const token = (0, generate6digits_1.generateRandom6DigitNumber)();
    const expires = new Date(new Date().getTime() + 3600 * 1000);
    const existingToken = await prismadb_1.prismadb.passwordResetToken.findFirst({
        where: {
            email,
        },
    });
    ...
    const passwordResetToken = prismadb_1.prismadb.passwordResetToken.create({
        data: {
            token,
            email,
            expires,
